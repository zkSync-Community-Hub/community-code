...
- Create a `getSignature()` function in your backend server(recommended) that signs required data with the help of signer.
...
- For the below code, you will need to create .env file with `ZKSYNC_RPC_URL` & `SIGNER_PRIVATE_KEY`.

- The `SIGNER_PRIVATE_KEY` is the private key of the signer address added in the paymaster in [the above step #2.](./permissionless-paymaster#_2-deposit-gas-funds-and-add-the-signer-address)

```javascript
// Example code
import {ethers, BigNumber} from "ethers";
import {BigNumber, Contract, Wallet, provider} from "zksync-ethers";
