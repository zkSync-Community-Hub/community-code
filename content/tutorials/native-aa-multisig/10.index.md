...
1. Add the ZKsync and OpenZeppelin contract libraries:
    ...
    ```shell [npm]
    npm install -D @matterlabs/zksync-contracts @openzeppelin/contracts@4.9.5
    ```
...
to allow interaction with system contracts:
    ...
    :code-import{filePath="multisig/hardhat.config.ts"}
    ```
...
1. In the `deploy` folder, create the file `deploy-factory.ts` and copy/paste the following code.
    ...
    :code-import{filePath="multisig/deploy/deploy-factory.ts"}
    ```
...
:code-import{filePath="multisig/deploy/deploy-multisig.ts:min-script"}
}
```
...
```ts [deploy-multisig.ts]
:code-import{filePath="multisig/deploy/deploy-multisig.ts:send-funds"}
```
...
```ts [deploy-multisig.ts]
:code-import{filePath="multisig/deploy/deploy-multisig.ts:create-deploy-tx"}
```
...
```ts [deploy-multisig.ts]
:code-import{filePath="multisig/deploy/deploy-multisig.ts:tx-gas"}
```
...
```ts [deploy-multisig.ts]
:code-import{filePath="multisig/deploy/deploy-multisig.ts:sign-tx"}
```
...
```ts [deploy-multisig.ts]
:code-import{filePath="multisig/deploy/deploy-multisig.ts:broadcast-tx"}
```
...
1. Copy/paste the following code into the deployment file, replacing the `<FACTORY-ADDRESS>` placeholder with the deployed contract address.
    ...
    ```ts [deploy-multisig.ts]
    :code-import{filePath="multisig/deploy/deploy-multisig.ts"}
    ```
