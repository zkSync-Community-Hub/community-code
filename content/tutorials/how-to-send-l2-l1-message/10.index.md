...
It is impossible to send transactions directly from L2 to L1. Instead, you can send arbitrary-length messages from
zkSync Era to Ethereum, and then handle the received message on Ethereum with an L1 smart contract.
...
Verification and confirmation is possible using Ethereum data. However, zkSync Era has an efficient [request proof function](#prove-the-result) which does the same.
::
...
3. Install the `zksync-contracts` package:
...
4. Install `zksync-ethers` and other Node.js dependencies:
...
5. In the root folder create an `.env` file with private key of wallet to use:

```js
"RICH_WALLET_PRIV_KEY=0x..";
```

6. Create a `send-message.ts` file in the root directory with the next script:
