---
title: How to test smart contracts
...
In this tutorial, the contract functionality is tested using the [zkSync Era Test Node](../../test-and-debug/era-test-node.md). To start local node we use the **hardhat-zksync-node** plugin to integrate this functionality within the Hardhat project.
...
The `hardhat-zksync-node` is included in `hardhat-zksync` so we just need to install the latest one:
