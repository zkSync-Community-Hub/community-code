...
::callout{icon="i-heroicons-exclamation-circle"}
Only the [bootloader](../../../developer-reference/system-contracts.md#bootloader) is allowed to call the `validateAndPayForPaymasterTransaction` and `postTransaction` functions. To implement that, the `onlyBootloader` modifier is used on these functions.
::
...
The [validation steps](../../../developer-reference/account-abstraction.md#the-validation-step) ensure that the paymaster won't throttle if the first storage read which has a different value from the execution on the API is a storage slot that belongs to the user.</br>
