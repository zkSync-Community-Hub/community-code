...
use to interact with on our frontend.
...
Move back into main project folder and create a frontend folder.
...
Then, move into the `frontend` folder and install the project dependencies.
...
Update the .env file in the `frontend` folder to add the `GREETER_CONTRACT_ADDRESS` with the deployed contract address from the previous section.
...
Navigate to [`http://localhost:5173/`](http://localhost:5173/) in a browser to see the application running.
