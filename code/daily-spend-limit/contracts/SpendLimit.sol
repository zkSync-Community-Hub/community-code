...
contract SpendLimit {
  ...
  function _checkSpendingLimit(address _token, uint _amount) internal {
    ...
    // ANCHOR: limit-is-enabled
    // return if spending limit hasn't been enabled yet
    if (!limit.isEnabled) return;
    // ANCHOR_END: limit-is-enabled

    uint timestamp = block.timestamp; // L2 block timestamp

    // ANCHOR: limit-available
    ...
    // ANCHOR_END: limit-available

    // ANCHOR: exceeds-limit
    ...
    limits[_token] = limit;
    // ANCHOR_END: exceeds-limit
  }
}
// ANCHOR_END: check-limit
