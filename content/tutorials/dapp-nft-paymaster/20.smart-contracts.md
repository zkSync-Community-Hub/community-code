...
[here](../../tooling/hardhat/getting-started.md#hardhat-configuration).
...
::callout{icon="i-heroicons-light-bulb"} A heads up! Make sure your account has zkSync Era Sepolia Testnet or Sepolia
ETH to successfully deploy the contracts. ::

2. Navigate to `/contracts` directory and open up the `ERC721.sol` contract. We will implement the missing logic for the
   `mint` function.
...
- Only the [bootloader](../../../zk-stack/components/zkEVM/bootloader.md) is allowed to call the
...
As described in [the paymaster documentation](../../developer-reference/account-abstraction.md#paymasters), there are
...
2. Next, we check the price of transaction fees, and transfer the correspondent gas fee from the paymaster to the
...
2. Deploy the contracts to zkSync Era Testnet. This will also programmatically verify the contracts on zkSync Era
   Testnet.
