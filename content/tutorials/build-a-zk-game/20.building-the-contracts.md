...
description: Build a ZK game contract.

---

## Project Setup

Clone the starting template and install the dependencies:
...
Move into the `contracts` folder that was generated:
...
Run the command below to move the `GeneralPaymaster` contract to the `contracts/contracts` folder,
...
Modify the `defaultNetwork` on line 8 inside the `hardhat.config.ts` file to deploy to a local in-memory node:
...
Create a new file for our game contract:
...
Copy and paste the full contract into the file:
...
Our prover program will have _private inputs_ and _public outputs_.
...
- `playerHighScores`: stores the high score for each player.
- `topPlayers`: keeps track of the players with the top 10 scores.
...
Create new three new deploy files inside the `contracts/deploy` folder for the verifier, paymaster, and game contracts:
...
Edit your `contracts/package.json` file to add the deploy scripts:
...
Next, compile and deploy the contract from inside the `zk-game-tutorial-template/contracts` folder:
...
Save the deployed contract address somewhere to use later.

### Deploying the Paymaster Contract

Next, let's deploy and fund the paymaster contract:
...
Save this deployed address as well.
