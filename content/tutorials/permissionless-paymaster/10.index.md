...
- Some background knowledge on the concepts covered by the tutorial would be helpful too. Have a look at the following
  ...
  - [Transaction lifecycle](https://docs.zksync.io/zk-stack/concepts/transaction-lifecycle#eip-712-0x71) on zkSyn Era.
...
The similar interation code is [available on GitHub here](https://github.com/ondefy/permissionless-multisigner-paymaster/blob/main/deploy/interact.ts).

::callout{icon="i-heroicons-exclamation-circle"}
This tutorial doesnot contain the entire frontend code require for the Dapp.
...
![one-to-many](/images/permissionless-paymaster/manager-signer.jpg)
...
3. Signer key signs on this paymaster data & returns the signature and signer address to the Dapp's frontend
...
6. Paymaster validates the signature, identifies the manager related to signer, deduct gas fees from the manager's balance & pay for user transaction.
...
3. Signer can call `selfRevokeSigner` to revoke themselves from signing.
